...
var SimpleGraph = function(elemid, options) {
  ...
  this.cx = this.chart.clientWidth || $(this.chart).width();
  this.cy = this.chart.clientHeight || $(this.chart).height();
...
SimpleGraph.prototype.plot_point = function(x,y) {
  this._points || (this._points = []);  
  this._points.push({x:x,y:y});
  this.plot_points();
};

SimpleGraph.prototype.plot_points = function(points) {
  ...
  this.line = d3.line()
      .x(function(d, i) { return that.x(that.points[i].x); })
      .y(function(d, i) { return that.y(that.points[i].y); });
