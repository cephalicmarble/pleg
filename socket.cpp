...
/**
 * @brief Socket::setSocketDescriptor : threaded implementation detail
 * @param socketDescriptor qintptr
 * @param socketState SocketState
 * @param openMode OpenMode
 * @return bool success
 */
bool Socket::setSocketDescriptor(qintptr socketDescriptor, SocketState socketState, OpenMode openMode)
{
    SOCKETLOCK
    connectSlots();
    if(!QAbstractSocket::setSocketDescriptor(socketDescriptor,socketState,openMode))
    {
        goto error;
    }
    open(ReadWrite);
    ...
    return true;
error:
    qDebug() << error() << " failed setSocketDescriptor";
    emit error(error());
    return false;
}
